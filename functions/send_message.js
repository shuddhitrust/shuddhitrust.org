...
exports.handler = function(event, context, callback){
  const { SENDGRID_EMAIL_API_KEY, RECAPTCHA_SECRET, SEND_TO_EMAIL } = process.env;
  ...
  request(verifyUrl, (err, reponse, body) => {
    ...
    if (body.success !== undefined && body.success) {
        ...
        let emailContentHTML = `<html><body><p><b>${formData.name} (${
          formData.email
        })</b>:</p><hr><p><b>${formData.subject}</b></p><p>${
          formData.content
        }</p><br />
        ...
        Subject: [shuddhifund.org] ${formData.subject}
    ...
    .then(() => {
       console.log(`Mail from ${mailOptions.from} is sent!`);
        ...
        console.error('Something went wrong. Details => ',`${error.code} :${error.message}`);
        send(error.code,{ success: false, msg: error.message });
    });
    } else {
      console.log("Recaptcha validation failed...");
      send(422,{ success: false, msg: `Captcha verification failed! Details => ${body['error-codes'][0]}`});
    }
  });
  }
